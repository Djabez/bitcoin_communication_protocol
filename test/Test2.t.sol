...
contract EndToEnd is Test {
    ...
    address public jabez = makeAddr("jabez");
    ...
    function testEndToEnd() external {
        ...
        assertEq(purchaseToken.balanceOf(bob),45e18);

        vm.stopPrank();

        vm.startPrank(jabez);
        purchaseToken.mint{value: 3e18}();
        purchaseToken.approve(address(secondaryMarket), higherBidPrice);
        secondaryMarket.submitBid(address(ticketNFT), id, higherBidPrice, "Jabez");
        
        assertEq(
            secondaryMarket.getHighestBid(address(ticketNFT), id),
            higherBidPrice
        );
        assertEq(secondaryMarket.getHighestBidder(address(ticketNFT), id), jabez);
        assertEq(purchaseToken.balanceOf(bob),200e18);
        assertEq(purchaseToken.balanceOf(address(secondaryMarket)), higherBidPrice);
        ...
        assertEq(purchaseToken.balanceOf(jabez),300e18);
